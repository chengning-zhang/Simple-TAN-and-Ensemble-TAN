...
class STAN_TAN_bagging(Bayes_net):
  ...
  def fit(self,X,y,M): 
    ...
    self.models_, self.p_ = models, p
    self.training_time_ = training_time/p # after parallel, only consider average of p TAN, ignore STAN since it takes less time than TAN
    self.is_fitted_ = True
    self.classes_ = model.classes_
    return self
